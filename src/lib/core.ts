...
declare global {
  let __lastStem: Record<string, string> | undefined
}
...
export async function runShrinkEngine(
  ...
  const primerPool =
    signal === 'high' ||
    ['danger', 'self-harm'].includes(detectLens(prompt))
      ? professionalPrimers
      : friendlyPrimers
  const primer =
    primerPool[Math.floor(Math.random() * primerPool.length)]
  ...
  const profileContext = userProfile
    ? `The user is ${userProfile.name ?? 'Anonymous'}, currently feeling ${
        (userProfile.emotionalTone ?? []).join(', ') ||
        'varied emotions'
      }.\n\n`
    : ''
  ...
  const responseToneTags =
    await inferToneTagsFromText(response_text)
